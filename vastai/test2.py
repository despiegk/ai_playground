...
# data2 = data["train"]["questions"].shuffle().map(generate_and_tokenize_prompt)
data2 = []

for x in data["train"]["questions"][0]:
    # pprint(x)
    if "answer" in x:
        if "question" in x:
            full_prompt = generate_prompt(x)
            pprint(full_prompt)
            tokenized_full_prompt = tokenizer(full_prompt, padding=True, truncation=True)
            data2.append(tokenized_full_prompt)
